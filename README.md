...
- La mayoría de colecciones se leen sin filtros complejos, pero los pronósticos revelados de `MatchesPage` se acotan por `jornada` + `kickoff` a la vez para no tener que leer los de toda la temporada en cada visita — eso sí necesita un índice compuesto, declarado en `firestore.indexes.json` y desplegado con `firebase deploy --only firestore:indexes`.
